...
inline float GetLinearDepth01(float2 uv) {
    float depth = Linear01Depth(SAMPLE_TEXTURE2D_X_LOD(_CameraDepthTexture, sampler_PointClamp, uv, 0).r, _ZBufferParams);
    return depth;
}
...
inline float3 GetSSCoords(float3 wpos) {
    ...
    coords.z = Linear01Depth(coords.z, _ZBufferParams);
    return coords;
}
...
half4 FragContactShadows(Varyings input) : SV_Target {
    ...
    float maxDist = step * SAMPLE_COUNT;
